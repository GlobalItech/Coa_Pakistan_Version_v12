...
#     parent_path=fields.Integer(index=True)
#     
class AccountAccount(models.Model):
    ...
    #parent_id = fields.Many2one('account.group','Parent Account',ondelete="set null")
    child_ids = fields.One2many('account.account','parent_id', 'Child Accounts')
    parent_left = fields.Integer('Left Parent', index=1)
    #parent_right = fields.Integer('Right Parent', index=1)
    #parent_path=fields.Many2one('account.group','parent_path')
    parent_path = fields.Char(index=True)
    #parent_path=fields.Integer('Parent path',index=1)
...
class AccountJournal(models.Model):
    ...
    def _prepare_liquidity_account(self, name, company, currency_id, type):
        ...
        #replaced
        code_digits = company.transfer_account_code_prefix or 0
        if type == 'bank':
            account_code_prefix = company.bank_account_code_prefix or ''
        else:
            account_code_prefix = company.cash_account_code_prefix or company.bank_account_code_prefix or ''

        liquidity_type = self.env.ref('COA_pak.data_account_type_view')
